...
- Level-10 opening of the Hard Bottle through its opening flourish, which consumes the bottle and grants three uses of 🫙🌿上薬草のジャム, plus that jam's prepared-state full recovery.
...
- Opening the Hard Bottle at level 10 is one-time: the bottle is lost and 🫙🌿上薬草のジャム is granted with `RPG.Config.HIGH_HERB_JAM_MAX_USES` (3) uses. The inventory shows it as `残り/最大` and states the remaining uses in its description.
