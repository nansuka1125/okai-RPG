...
innSystem = {
    ...
    showInnMidnightMenu: function () {
        ...
        container.innerHTML = '';
        container.classList.add('btn-grid');
        container.style.display = 'grid';

        const addChoice = (text, action, extraClass = '') => {
    ...
    finishInnMidnight: function () {
        ...
        const container = document.getElementById('action-buttons');
        if (container) {
            container.classList.remove('btn-grid');
            container.innerHTML = '';
    ...
    playFirstAmberAppraisal: function () {
        ...
        RPG.State.dialogueQueue = [
            ...
            { text: "琥珀商「珍しい琥珀には、装備できるやつもある。アイテム欄から試してみな」" },
            { text: "（琥珀はブローチから装備できます）", type: "ambient", color: "#555555", fontSize: "13px" },
            {
                text: null,
                action: () => {
                    RPG.State.mode = "base";
